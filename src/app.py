...
def multi():
    if request.method == 'POST':
        ...
        image = Image.open(image_file)
        processed = preprocess_multiClass_image(image)
        prediction = multiModel.predict(processed)
        predicted_class = np.argmax(prediction)

        labels = {0: 'Glioma', 1: 'Meningioma', 2: 'Pituitary'}
        return jsonify({'result': labels.get(predicted_class, 'Unknown')})
...
def segment():
    if request.method == 'POST':
        ...
        return jsonify({'img_base64': img_base64, 'tumor_percentage': tumor_percentage})
